  ...
  subroutine vz_integral(vy,integral,s,omega,t)
        ...
        if (vz_istep==int_nsteps) then
           ...
           if (s==2) then
              ...
              if (isnan(bj)) bj = 1.
              integral = &
                 bj**2*sqrt(2.0/pi)*vy**2*exp(-vy**2/2.)*(integral+res_v1+res_v2)*sqrt(2./A)
              !print*, 'vz_integral, vy, omega'
              !print*, integral, vy, omega
           end if
        end if
  ...
  function integrand(vz_prime,vy,omega,shift_vz,s,t)
        ...
        if (s==2) then
                ...
                omega_star_t = -theta*(-3./2.+vy**2/2.)&
                        *tprime*ky
                omega_d = -theta*kv/A*vy**2
                coll_te = zi*nu0*A/vy**3
