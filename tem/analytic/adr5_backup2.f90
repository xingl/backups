...
subroutine quadratic_solution(omega_quadr, omega_quadr_a)
        ...
        complex (kind=8) :: test1, test2, test3
        integer(kind=8) :: t,s
        ...
        test1 = fprime_i*ky-2.*omd*ky/A
        test2 = (fprime_i*ky-2.*omd*ky/A)**2 - &
                8.*(fprime_i*ky+tprime_i*ky)*omd*ky/A
        test3 = sqrt(test3)
        omega1 = (-test1+test3)/2.
        omega2 = (-test1-test3)/2.
...
subroutine recur_quadratic_soln(omega_quadr_recur)
        ...
        complex (kind=8) :: omega1, omega2
        complex (kind=8) :: test1, test2, test3, test4
        integer (kind=8) :: s,t
        ...
        s = 2   !fluid ion limit
        t = 1 
        call vy_integral_simpson(this_omega,vy_1,vy_2,test1,s,t)
        t = 2
        call vy_integral_simpson(this_omega,vy_1,vy_2,test2,s,t)
        t = 3 
        call vy_integral_simpson(this_omega,vy_1,vy_2,test3,s,t)

        test4 = -1./theta - Zeff + test1

        omega1 = (-test2+sqrt(test2**2-4.*test1*test3))/2./test4
        omega2 = (-test2-sqrt(test2**2-4.*test1*test3))/2./test4
        ...
        if (abs(this_omega-next_omega)>sec_tol) then
                ...
                test4 = -1./theta - Zeff + test1 + t_e*this_int_te

                omega1 = (-test2+sqrt(test2**2-4.*test4*test3))/2./test4
                omega2 = (-test2-sqrt(test2**2-4.*test4*test3))/2./test4
...
subroutine recur_linear_soln(omega_linr_recur)
        ...
        this_omega = 0.
        next_omega = -(fprime_i*ky*(1-ky**2+ky**4/2.)+&
                tprime_i*ky*(-ky**2+ky**4))/(1+ky**2-ky**4/2.)
        if (abs(this_omega-next_omega)>sec_tol) then
                this_omega = next_omega
                print*, 'omega_linear_recur'
                print*, this_omega
                ...
                call vy_integral(this_omega,this_int_te,s)
                next_omega = -(fprime_i*ky*(1-ky**2+ky**4/2.)+&
                    tprime_i*ky*(-ky**2+ky**4))/(1+ky**2-ky**4/2.-t_e*this_int_te)
        end if
...
subroutine dispersion_relation(omega, rhs)
        ...
        if (t_e==0.) then
            ...
            integral_te_s_nu_a = sqrt(2.*eps/(1.+eps))*(1.-(-theta*fprime_i*ky)/omega)
            integral_te_l_nu_a = sqrt(2.*eps/(1.+eps))*(-zi)*eps/3./nu0*&
