...
subroutine dispersion_relation(omega, rhs)
        ...
        complex (kind=8) :: integral_te_l_nu2
        integer(kind=8) :: s
        ...
        if (t_e==0.) then
            ...
            write (Dmixing_unit, &
            '(11e12.4)') nu0,omega,integral_te,integral_te_s_nu,&
                        integral_te_l_nu, integral_te_l_nu2
        end if
        if (na_e==0.) then
            integral_e = 0.
        else
...
subroutine vy_integral_simpson(omega,vy_a,vy_b,integral,s,t)
  ...
  function integrand(vz_prime,vy,omega,shift_vz,s,t)
        ...
        if (t==0.and.(.not.s==4).and.(.not.s==5)) then
                int_tmp = exp(-vz**2/2.)*&
...
subroutine init_output_files
        ...
        write (Dmixing_unit,'(11a12)') "nu0","omega","full_int",&
                "s_nu_int","l_nu_int","l_nu_int2"
