...
!        print*, this_omega
        ...
        if (abs(this_omega-next_omega)>sec_tol) then
                ...
                term4 = -1./theta - 1. + term1 + t_e*this_int_te
...
subroutine linear_solution(omega_linr, omega_linr_a)
        ...
        term3 = 1./theta + 1. - term1

        omega1 = term2/term3

        omega_linr = omega1

        omega_linr_a = -(fprime_i*ky*(1-ky**2+ky**4/2.)+&
  ...
  subroutine vz_integral(vy,integral,s,omega,t)
        ...
        if (vz_istep==int_nsteps) then
           ...
           if (s==2) then
              ...
              if (isnan(bj)) bj = 1.
              integral = &
                 sqrt((2.*eps)/(1.+eps))*sqrt(2.0/pi)*vy**2*exp(-vy**2/2.)*integral
           end if
        end if
