...
contains
 ...
 subroutine kx_scan (kx_maxgam,omega_maxgam,kx_maxDmix,Dmixing_max)
        ...
        complex (kind=8) :: root_tmp, om_linr, om_quadr
          ...
          do ikx = ikx_ref(1), 1, -1
             ...
             if (sec_istep==sec_nsteps.or.isnan(real(root_tmp))) then
              ...
              if (aimag(root_tmp)>aimag(omega_kx_scan(ikx))) &
                omega_kx_scan(ikx) = root_tmp
              call quadratic_solution(om_quadr)
              call linear_solution(om_linr)
              write (out_unit, '(14e12.4)') tprime_i,fprime_i,tprime_e,fprime_e,&
                             omd,kx,ky,kz,omega_kx_scan(ikx),om_linr,om_quadr
             end if
          end do
          write (out_unit,*) 
...
subroutine linear_solution(omega_linr)

        implicit none
        
        complex(kind=8), intent(out) :: omega_linr
        complex(kind=8) :: omega1, omega2
        complex (kind=8) :: term1, term2, term3
        integer(kind=8) :: t,s
...
subroutine dispersion_relation(omega, rhs)
        ...
        complex (kind=8) :: integral_te, integral_te_s_nu, integral_te_l_nu
        integer(kind=8) :: s
        ...
        if (t_e==0.) then
            ...
            print*, 'integral_te'
            print*, integral_te
            print*, 'eps, omega, theta, fprime_i, tprime_e, ky'
            print*, eps, omega, theta, fprime_i, tprime_e, ky
            integral_te_s_nu = sqrt(2.*eps)*(1.-(-theta*fprime_i*ky)/omega)
            integral_te_l_nu = sqrt(2.*eps)*(-zi)*eps/3./nu0*&
                (8.*sqrt(2./pi)*(omega-(-theta*fprime_i*ky))-12.*sqrt(2./pi)&
                *(-theta*tprime_e*ky))
            print*, 'integral_te_s_nu'
            print*, integral_te_s_nu
            print*, 'integral_te_l_nu'
            print*, integral_te_l_nu
        end if
        if (na_e==0.) then
            integral_e = 0.
  ...
  subroutine vz_integral(vy,integral,s,omega,t)
        ...
        if (vz_istep==int_nsteps) then
           ...
           if (s==2) then
              ...
              if (isnan(bj)) bj = 1.
              integral = &
                 sqrt(2*eps)*bj**2*sqrt(2.0/pi)*vy**2*exp(-vy**2/2.)*integral
           end if
        end if
...
subroutine init_output_files
        ...
        !        "omd","kx","ky","kz","omega","gamma"
        write (out_unit,'(14a12)') "tprime_i","fprime_i","tprime_e","fprime_e",&
                "omd","kx","ky","kz","omega","gamma","omlin","gmlin","omquad","gmquad"
end subroutine
