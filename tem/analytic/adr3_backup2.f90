...
subroutine quadratic_solution(omega_quadr, omega_quadr_a)
        ...
        print*,'fprime_i,ky,omd,A,tprime_i'
        print*,fprime_i,ky,omd,A,tprime_i
        !test1 = 2.
        test2 = fprime_i*ky-2.*omd*ky/A
        test3 = (fprime_i*ky-2.*omd*ky/A)**2 - &
                8.*(fprime_i*ky+tprime_i*ky)*omd*ky/A
        test1 = sqrt(test3)
        print*,'A,B,Delta'
        print*,test1,test2,test3
        !omega1 = (-(fprime_i*ky-2.*omd*ky/A)+ &
        !        sqrt((fprime_i*ky-2.*omd*ky/A)**2 - &
        !        8.*(fprime_i*ky+tprime_i*ky)*omd*ky/A))/2.
        !omega2 = (-(fprime_i*ky-2.*omd*ky/A) - &
        !        sqrt((fprime_i*ky-2.*omd*ky/A)**2 - &
        !        8.*(fprime_i*ky+tprime_i*ky)*omd*ky/A))/2.
        omega1 = (-test2+test1)/2.
        omega2 = (-test2-test1)/2.
...
subroutine recur_quadratic_soln(omega_quadr_recur)
        ...
        complex (kind=8) :: omega_tmp, omega3,omega4
        integer (kind=8) :: s

        this_omega = 0.
        call quadratic_solution(omega_tmp, next_omega)
        print*, 'omega_tmp,next_omega'
        print*, omega_tmp, next_omega
        ...
        if (abs(this_omega-next_omega)>sec_tol) then
                ...
                omega3 = -(fprime_i*ky-2.*omd*ky/a)/2./(1.-t_e*this_int_te) + &
                        sqrt((fprime_i*ky-2.*omd*ky/a)**2 - &
                        8*(1.-t_e*this_int_te)*(fprime_i*ky+tprime_i*ky)*&
                        omd*ky/a)/2./(1.-t_e*this_int_te)
                omega4 = -(fprime_i*ky-2.*omd*ky/a)/2./(1.-t_e*this_int_te) - &
                        sqrt((fprime_i*ky-2.*omd*ky/a)**2 - &
                        8*(1.-t_e*this_int_te)*(fprime_i*ky+tprime_i*ky)*&
                        omd*ky/a)/2./(1.-t_e*this_int_te)
                print*, 'omega3,4'
                print*, omega3, omega4
                if (aimag(omega3)>aimag(omega4)) then
                     next_omega = omega3
                else
                     next_omega = omega4
                end if
