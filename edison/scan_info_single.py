...
if 'x_local' in pars and not pars['x_local']:
    scan_info = np.zeros((numscan_tot,12),dtype='float64')
    for i in range(numscan_tot):
        ...
        if os.path.isfile('field_'+scan_num):
            ...
            phi = field.phi() 
            apar = field.apar() 
            cfunc,zed,corr_len=my_corr_func_complex(phi[:,0,imax[1]],phi[:,0,imax[1]],zgrid,show_plot=False)
            scan_info[i,6] = np.nan
            scan_info[i,7] = corr_len
            #Modified to here!!!!!!!
            parity_factor_apar = np.abs(np.sum(apar[:,0,imaxa[1]]))/np.sum(np.abs(apar[:,0,imaxa[1]]))
            scan_info[i,8] = parity_factor_apar
            parity_factor_phi = np.abs(np.sum(phi[:,0,imax[1]]))/np.sum(np.abs(phi[:,0,imax[1]]))
            ...
            for j in range(field.nx):
                ...
                phi_bnd[1,:,j] = phi_bnd[-3,:,j]*np.e**(phase[j])
                gradphi[:,0,j] = fd_d1_o4(phi_bnd[:,0,j],zgrid_ext)
                gradphi[2:-2:,0,j] = gradphi[2:-2,0,j]/np.pi/(geometry['jacobian'][:,j]*geometry['Bfield'][:,j])
...
#local scan
    ...
    for i in range(numscan_tot):
        ...
        if os.path.isfile('omega'+suffix):
            omega0 = np.genfromtxt('omega'+suffix)
            if omega0.any() and omega0[1] != 0.0:
                scan_info[i,4]=omega0[1]
                scan_info[i,5]=omega0[2]
            elif calc_grs:
            ...
            scan_info[i,4]=np.nan
            scan_info[i,5]=np.nan
        if calc_omega and os.path.isfile('field'+suffix):
            call(['calc_omega_from_field.py',scan_num])
            om = np.genfromtxt('omega_'+scan_num)
            scan_info[i,4]=om[1]
            scan_info[i,5]=om[2]
        
        if os.path.isfile('field'+suffix):
            field = fieldfile('field'+suffix,pars0)
            field.set_time(field.tfld[-1])
            fntot = field.nz*field.nx
            ...
            #KBM test with E||
            gpars,geometry = read_geometry_local(pars0['magn_geometry'][1:-1]+suffix)
        ...
        if os.path.isfile('nrg'+suffix):
            if nspec==1:
               ...
               tn,nrg1,nrg2,nrg3=get_nrg0(suffix,nspec=nspec)
                ...
                sys.exit("Not ready for nspec>4")
        else:
            scan_info[i,10] = np.nan
    
    f=open('scan_info.dat','w')
    p = open('parameters'+suffix,'r')
    lines = p.read()
    p.close()
    lines = lines.split('\n')
    for line in lines:
...
#os.chdir('../')
call(['plot_scan_info.py',suffix_in])
