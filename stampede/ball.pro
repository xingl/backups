...
;  IF NOT KEYWORD_SET(*e.reim) THEN *e.reim = 0
  ...
  nxmod = 1L * (par.x_local ? ((par.nx0 - 1) / 2) * 2 + 1 : par.nx0)
  ...
  IF nxmod GT 1 THEN BEGIN
    jbar = LONARR(nxmod)
    jbar[0] = par.nx0 / 2 + 1 + LINDGEN(nxmod/2)
    jbar[nxmod/2] = LINDGEN(nxmod-nxmod/2)
    ...
    IF reim THEN BEGIN
      ...
      FOR k = 0L, par.nz0 -1 DO BEGIN
        FOR j = 0L, nky - 1 DO BEGIN
    ...
    FOR isp = 0, gui.out.n_spec_sel - 1 DO BEGIN
      FOR v = 0, (*i).n_vars - 1 DO BEGIN
        FOR modeind = 0, nky - 1 DO BEGIN ; -- start mode loop
          FOR j = 0L, (*i).nxmod - 1 DO BEGIN
            exp_fac = EXP(-2.0D0*!DPI/par.lx*COMPLEX(0.0,1.0)*(*i).jbar[j]*xvar)
            FOR k = 0L, par.nz0 - 1 DO varb_temp[k+j*par.nz0,modeind,isp] = $
              nx0_inv * TOTAL((*mom[isp,(*i).vars[v]].sxky)$
              [LINDGEN((*i).nxmod),(*i).kyind[modeind],k]*exp_fac)
          ENDFOR
...
PRO ball_theta_miller, theta
;computes theta as used in the Miller definition for the Z coordinate
  COMMON global_vars

  maxit = 10000

  nth = N_ELEMENTS((*series.geom).Z)

  torR = TOTAL((*series.geom).R*$
      (*series.geom).jacobian) / TOTAL((*series.geom).jacobian)
  torZ = TOTAL((*series.geom).Z*$
      (*series.geom).jacobian) / TOTAL((*series.geom).jacobian)

  diffZ = (*series.geom).Z-torZ ;par.major_Z*par.Lref
  diffR = (*series.geom).R-torR ;par.major_R*par.Lref

  theta = ASIN(diffZ/(par.x0*par.minor_r*par.Lref*par.kappa))
  theta[WHERE((diffZ LT 0) AND (diffR LT 0))] = $
     -theta[WHERE((diffZ LT 0) AND (diffR LT 0))]-!PI
  theta[WHERE((diffZ GE 0) AND (diffR LT 0))] = $
     -theta[WHERE((diffZ GE 0) AND (diffR LT 0))]+!PI
  IF theta[0] GT 0 THEN theta[0]-= 2.0*!PI
  theta[WHERE(FINITE(theta) EQ 0)] = $
     0.5*!PI*SIGNARR(theta[WHERE(FINITE(theta) EQ 0)-1])
  B = theta

  ; use theta w/o squareness as initialization for Newton method
  FOR ith = 0, nth - 1 DO BEGIN
    FOR it = 0, maxit DO BEGIN
      f = theta[ith] + par.zeta * SIN(2*theta[ith]) - B[ith]
      fprime = 1 + 2 * par.zeta * COS(2*theta[ith])

      IF ABS(f) LT 10.0 * (Machar()).eps THEN BREAK $
        ELSE theta[ith] -= f / fprime
    ENDFOR

    IF it GE maxit THEN $
      printerror, 'Warning: theta_Miller computation not converged'
  ENDFOR
...
PRO ball_output, diag
  ...
  ;construct poloidal angle
  torR = TOTAL((*series.geom).R*$
    (*series.geom).jacobian) / TOTAL((*series.geom).jacobian)
;  torR = par.major_R*par.Lref
  torZ = TOTAL((*series.geom).Z*$
      (*series.geom).jacobian) / TOTAL((*series.geom).jacobian)

  IF (par.x_local EQ 1 AND par.y_local EQ 1) THEN $
     theta_pol = ATAN((*series.geom).Z-torZ,(*series.geom).R-torR) $
  ELSE theta_pol = ATAN((*series.geom).Z[0,*]-torZ,$
                        (*series.geom).R[0,*]-torR)
  IF theta_pol[0] GT 0 THEN theta_pol[0]-= 2.0*!PI

  with_miller = (STRPOS(par.magn_geometry,'miller') GE 0)
  IF (with_miller) THEN ball_theta_miller, theta_mil
    ...
    FOR n = 0, (gui.out.res_steps AND NOT (*i).use_pod) * $
      ...
      FOR v = 0, n_varplots - 1 DO BEGIN
        ...
        FOR modeind = 0, nky - 1 DO BEGIN ; --- y mode loop
          ...
          ;pindz: number of total connections
          pindc = 1L * ((((par.nx0 - 1) / 2) / bigN) * 2 + 1)
          ; pinddim: number of points in the final curve
          pinddim = 1L * pindc * par.nz0
          ...
          FOR p = 0, 2 * pindrange DO BEGIN
            pind[p*par.nz0+LINDGEN(par.nz0)] = $
              (reloffset + p * bigN) * par.nz0 + LINDGEN(par.nz0)
          ENDFOR
...
;          tppr = (2 * FINDGEN(pinddim) - pinddim) / par.nz0 * par.n_pol
          ...
          tpol = tppr
          tpolm = tpol
          FOR p = 0, pindc-1 DO BEGIN
             tppr[p*par.nz0:(p+1)*par.nz0-1] = (*par.z)/!PI+(-pindc/2+p)*2.0
             tpol[p*par.nz0:(p+1)*par.nz0-1] = theta_pol/!PI+(-pindc/2+p)*2.0
             IF (with_miller) THEN tpolm[p*par.nz0:(p+1)*par.nz0-1] = $
                theta_mil/!PI+(-pindc/2+p)*2.0
          ENDFOR
...
;            plotdata_ext = (TRANSPOSE(TEMPORARY(V_matrix_tz)))[*,0:(*i).n_pod-1]
          ...
          FOR p = 0, ((*i).n_pod > 1) - 1 DO BEGIN
            ...
            print_parity_fac = 0
            IF print_parity_fac THEN BEGIN
              ; Print mode parity factor, averaged over re/im and
              ; weighted by squared amplitude: -1 = fully odd, 1 = fully even.
              ; Use equal weights along theta. Ignore first point along theta.
              absdata = ABS(plotdata[1:*])^2
              parity_re = TOTAL(absdata*(REVERSE(FLOAT(plotdata[1:*]))/$
                FLOAT(plotdata[1:*]))) / TOTAL(absdata)
              parity_im = TOTAL(absdata*(REVERSE(IMAGINARY(plotdata[1:*]))/$
                IMAGINARY(plotdata[1:*]))) / TOTAL(absdata)
              parity = 0.5 * (parity_re + parity_im)
              PRINT, 'parity: ' + rm0es(parity,prec=3)
            ENDIF
            ...
            IF (*i).range[0] GE 0.0 THEN BEGIN ; plot zoomed region
              ...
              plotmin = MIN(ABS(plotdata[detail_range]),MAX=plotmax) > 1e-6
              IF (*i).reim THEN BEGIN
        ...
        IF NOT (*i).use_pod THEN BEGIN
          IF with_miller THEN set_output, diag, sp, $
            header=['theta','Re('+var_string2+')','Im('+var_string2+')',$
            'theta_pol','theta_M'], commentline=[['ky='+rm0es(kyreal)+$
            ((*i).norm?'; *** data normalized to theta=0 value ***':'')],$
            ['theta is the straight field line angle divided by pi'],$
            ['theta_pol is the poloidal angle divided by pi with '+$
            'respect to estimated (R,Z) center values'],$
            ['theta_M is the angle used in the Miller definition']],$
            dat=[[tppr],[FLOAT(plotdata)],[IMAGINARY(plotdata)],$
            [tpol],[tpolm]], append=(v GT 0) $
            ELSE set_output, diag, sp, header=['theta','Re('+var_string2+')',$
            'Im('+var_string2+')','theta_pol','theta_M','th_M2'], $
            commentline=[['ky='+rm0es(kyreal)+((*i).norm ? $
            '; *** data normalized to theta=0 value ***':'')],$
            ['theta is the straight field line angle divided by pi'],$
            ['theta_pol is the poloidal angle divided by pi with '+$
            'respect to estimated (R,Z) center values']],$
            dat=[[tppr],[FLOAT(plotdata)],[IMAGINARY(plotdata)],$
