...
#        sys.exit("Must have n_spec=2")
        ...
        time,nrge = get_nrg0(suffix,nspec=nspec,ncols=ncols)
        nrgi = np.zeros(np.shape(nrge))
    else:
        sys.exit("Must have n_spec = 1,2,3")

    #print  "1. nrgi[-1,0]/nrgi[-2,0]",nrgi[-1,0]/nrgi[-2,0]
#    if nrgi[-1,0]/nrgi[-2,0] < 1.0e-10:
    if nrge[-1,0]/nrge[-2,0] < 1.0e-10:
    ...
    if nspec == 1 :
        fit =  np.e**(2.0*avg_gr[5]*time[start_index:])*(nrge[-1,0]/np.e**(2.0*avg_gr[5]*time[-1]))
        err = abs(np.sum(nrge[start_index:,0]-fit[:])/np.sum(nrge[start_index:,0]))
    else :
...
#    print "Calculated growth rate:",avg_gr[0]
    print "Calculated growth rate:",avg_gr[5]
    print "Error:",err
    if err > 1.0e-2:
        if nspec == 1:
            plt.semilogy(time,nrge[:,0])
            plt.semilogy(time[start_index-500:],np.e**(2.0*avg_gr[5]*time[start_index-500:])*(nrge[-1,0]/np.e**(2.0*avg_gr[5]*time[-1])),'--',color='green')
            plt.show()
