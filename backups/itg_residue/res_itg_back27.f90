program res_itg
        ...
        integer :: out1_unit=103, out2_unit=104
        ...
        write (Dmixing_unit,'(7a12)') "tprime","fprime","omd","kx",&
                                "ky","kz","Dmixing"
        call open_output_file(out1_unit,'.dat1')
        write (out1_unit,'(8a12)') "tprime","fprime","omd","kx",&
                                "ky","kz","omega","gamma"
        call open_output_file(out2_unit,'.dat2')
        write (out2_unit,'(8a12)') "tprime","fprime","omd","kx",&
                                "ky","kz","omega","gamma"
              ...
              do itprime = 1, ntprime
                 ...
                 do iky = 1, nky
                    ...
                    do ikz = 1, nkz
                       ...
                       if (.not.aimag(omega_kx_max)==-9999.0) &
                           write (gamma_unit,&
                           '(8e12.4)') tprime,fprime,omd, &
                           kx_gam,ky,kz,omega_kx_max
                       if (.not.Dmixing_kx_max==-9999.0) &
                           write (Dmixing_unit, &
                           '(7e12.4)') tprime,fprime,omd, &
                           kx_Dmix,ky,kz,Dmixing_kx_max
                       if (.not.aimag(omega_kx_max)==-9999.0) then  
                           ikx_ref = minloc(abs(kx_base-kx_gam))
                           root = omega_kx_max
                           root_ky_kz = omega_kx_max
                       end if
        ...
        call close_output_file (out1_unit)
        call close_output_file (out2_unit)
        ...
        if (iky==1.and.ikz==1) then
          ...
          do ikx = ikx_ref(1), 1, -1
             ...
             if (sec_istep==sec_nsteps.or.isnan(real(root_tmp))) then
              ...
              root = seed1
!             else if (abs(aimag(root_tmp))<1.E-09 .or. abs(root_tmp)>10.) then
             else if (abs(aimag(root_tmp))<lower_bnd) then
              print*, 'No root is found.'
              root = seed1
              omega_grid(ikx) = 0. -9999.*zi
              ...
              print*, "Root is found:"
              print*, root_tmp    
              write (out2_unit, '(8e12.4)') tprime,fprime,&
                                        omd,kx,ky,kz,root_tmp
              write (out1_unit, '(8e12.4)') tprime,fprime,&
                                        omd,kx,ky,kz,root_tmp
              omega_grid(ikx) = root_tmp
              root = root_tmp
             end if
          end do
          do ikx = ikx_ref(1), n_base
             kx = kx_base(ikx)
             kv = ((1-omd_kx)*ky + omd_kx*kx)*omd
             print*, "kx,kv"
             print*, kx,kv
          ...
          ikx_maxgamma = maxloc(aimag(omega_grid))
          Dmixing_kx_scan = aimag(omega_grid)/(kx_base**2+ky**2)
          ikx_maxDmixing = maxloc(Dmixing_kx_scan)

          kx_maxgam = kx_base(ikx_maxgamma(1))
          omega = omega_grid(ikx_maxgamma(1))
          kx_maxDmix = kx_base(ikx_maxDmixing(1))
          ...
          allocate(Dmixing_kx_scan(n_base))
          allocate(root_kx_scan(2,n_base))
          allocate(ikx_maxgamma(1),ikx_maxDmixing(1),seed_ind(1))
          Dmixing_kx_scan = -9999.0
          root_kx_scan = 0.-9999.0*zi
          omega_grid = 0.-9999.0*zi
          print*, ikx_ref(1)
          do ikx = ikx_ref(1),1,-1
             kx = kx_base(ikx)
             kv = ((1-omd_kx)*ky + omd_kx*kx)*omd
...
!             print*, "kx,kv"
!             print*, kx,kv
!             if (aimag(omega_grid(ikx))==-9999.) then
!                seed1 = root_tmp
!                seed2 = seed1*seed_fr
!             else
!                seed1 = omega_grid(ikx)
!                seed2 = seed1*seed_fr
!             end if
!             print*,'seed1,seed2'
!             print*,seed1,seed2
...
!              print*, "Root is found:"
!              print*, root_tmp
!              root_kx_scan(2,ikx) = root_tmp
!             end if
!          end do

          do ikx = 1, n_base
!             if (aimag(root_kx_scan(1,ikx))>aimag(root_kx_scan(2,ikx))) then
!                if (.not.aimag(root_kx_scan(1,ikx))==-9999.) &
!                omega_grid(ikx) = root_kx_scan(1,ikx)
!                print*,1
!             else
!                if (.not.aimag(root_kx_scan(2,ikx))==-9999.) &
!                omega_grid(ikx) = root_kx_scan(2,ikx)
!                print*,2
!             end if
             !if (aimag(root_kx_scan(1,ikx))==-9999.) then
             !   omega_grid(ikx) = 
             !else
                omega_grid(ikx) = root_kx_scan(1,ikx)
             !end if
             if (.not.aimag(root_kx_scan(1,ikx))==-9999.) write (out1_unit,&
               '(8e12.4)')tprime,fprime,omd,kx_base(ikx),ky,kz,root_kx_scan(1,ikx)
             if (.not.aimag(root_kx_scan(2,ikx))==-9999.) write (out2_unit,&
               '(8e12.4)')tprime,fprime,omd,kx_base(ikx),ky,kz,root_kx_scan(2,ikx)
          end do
          write(out1_unit,*)
          write(out2_unit,*)
