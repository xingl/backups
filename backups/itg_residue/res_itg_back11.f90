              ...
              do itprime = 1, ntprime
                 ...
                 do iky = 1, nky
                    ...
                    do ikz = 1, nkz
                       ...
                       kx_lp = 0.; omega_lp = 0.; Dmixing_lp = 0.
                       if (iky==1.and.ikz==1) then
                          omega_grid = 0.
                          do nlp = 1,2
                             call kx_scan(nlp,kx_lp(nlp),omega_lp(nlp),&
                                  kx_lp_D(nlp),Dmixing_lp(nlp))
                             write(out_unit,*)
                          end do
                          write(out_unit,*)
                          ...
                          if (.not.maxval(aimag(omega_lp))==-9999.0) then  
                              ...
                              if (kv==0) root0 = omega_lp(ikx_lp_gamma(1))
                           end if
                       else
                          nlp = 1
                          call kx_scan(nlp,kx_lp(nlp),omega_lp(nlp),&
...
contains
 ...
 subroutine kx_scan (iloop,kx_maxgam,omega,kx_maxDmix,Dmixing)
        ...
        real (kind=8) :: kx_init
        integer (kind=8), dimension(:), allocatable :: kx_ind



        if (iky==1.and.ikz==1) then
           allocate(omega_kx_scan(nkx),Dmixing_kx_scan(nkx))
           allocate(kx_ind(1),ikx_maxgamma(1),ikx_maxDmixing(1))
           omega_kx_scan = -9999.0*zi+0.
          ...
          do ikx = 1, nkx
             ...
             if (steps==nstep.or.isnan(real(root))) then
              ...
              root = seed1
             else if (abs(aimag(root))<1.E-09) then
              print*, 'No root is found.'
              root = seed1
             else
              print*, "Root is found:"
              ...
              kx_ind = minloc(abs(kx_base-kx))
              omega_grid(kx_ind(1)) = root
             end if
          end do
          ...
          root = 0.
          allocate(omega_kx_scan(n_base),Dmixing_kx_scan(n_base))
          allocate(kx_ind(1),ikx_maxgamma(1),ikx_maxDmixing(1))
          omega_kx_scan = -9999.0*zi+0.
          Dmixing_kx_scan = -9999.0
          do ikx = 1,n_base
