program res_itg
        ...
        real(kind=8) :: cut_buffer, seed_fr
...
contains
 ...
 subroutine kx_scan (kx_maxgam,omega,kx_maxDmix,Dmixing)
        ...
        if (iky==1.and.ikz==1) then
          ...
          do ikx = 1,n_base
             ...
             if (sec_istep==sec_nsteps.or.isnan(real(root_tmp))) then
              print*, 'No root is found.'
              root_kx_scan(2,ikx) = real(root)+0.*zi
             else if (abs(aimag(root_tmp))<1.E-09 .or. abs(root_tmp)>10.) then
              print*, 'No root is found.'
              root_kx_scan(2,ikx) = real(root)+0.*zi
 ...
 subroutine rootfinder(sd1,sd2,rt,sec_istep)

        implicit none

        complex (kind=8), intent(in) :: sd1,sd2
        complex (kind=8), intent(out) :: rt
        integer (kind=8), intent(out) :: sec_istep
