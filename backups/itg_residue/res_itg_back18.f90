 ...
 subroutine kx_scan (kx_maxgam,omega,kx_maxDmix,Dmixing)
        ...
        if (iky==1.and.ikz==1) then
          ...
          do ikx = ikx_ref(1), 1, -1
             ...
             if (steps==nstep.or.isnan(real(root))) then
              print*, 'No root is found.'
              root = seed1
             else if (abs(aimag(root))<1.E-09 .or. abs(root_tmp)>10.) then
              print*, 'No root is found.'
              root = seed1
             else
              print*, "Root is found:"
          ...
          do ikx = ikx_ref(1),1,-1
             ...
             print*,'seed1,seed2'
             print*,seed1,seed2
             call rootfinder(seed1,seed2,root_tmp,steps)
             if (steps==nstep.or.isnan(real(root_tmp))) then
              print*, 'No root is found.'
              root_kx_scan(1,ikx) = 0.
              root = seed1
              exit
          ...
          do ikx = 1, n_base
             ...
             if (.not.aimag(omega_grid(ikx))==-9999.) write (out1_unit,&
               !'(8e12.4)') tprime,fprime,omd,kx_base(ikx),ky,kz,omega_grid(ikx)
               '(8e12.4)')tprime,fprime,omd,kx_base(ikx),ky,kz,root_kx_scan(1,ikx)
             if (.not.aimag(omega_grid(ikx))==-9999.) write (out2_unit,&
