...
subroutine quadratic_solution(omega)
        ...
        complex (kind=8) :: term1, term2, term3, term4, term5
        ...
        Delta = (-term2+term3)**2-4.*term1*(term4+term5)
        print*, 'Delta'
        print*, Delta

!        omega1 = (-(-term2+term3)+sqrt((-term2+term3)**2-4.*term1*(term4+term5)))/2./term1
        omega1 = (-(-term2+term3)+sqrt(Delta))/2./term1
        print*, 'omega1'
        print*, omega1
...
!        omega2 = (-(-term2+term3)-sqrt((-term2+term3)**2-4.*term1*(term4+term5)))/2./term1
        ...
        if (aimag(omega2)>aimag(omega1)) then
           omega = omega2
        else
           omega = omega1
        end if
...
subroutine vy_integral_simpson(omega,vy_a,vy_b,integral,s,t)
  ...
  subroutine read_input_file
        ...
        namelist / parameters / sec_nsteps,nkz,dkz,kz_start, &
                                ...
                                om2_re,om2_im,nu0,A,vi,int_tol,sec_tol,fr, &
