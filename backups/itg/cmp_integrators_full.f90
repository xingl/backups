program cmp_integrators_vy
        ...
        integer :: nvy, nvz
        integer (kind=8) ::  nsteps_vy, nsteps_vz, nsteps_in
        integer :: iy, iz
        real (kind=8) :: lvy, lvz
        real (kind=8) :: ky, kz
        ...
        integer :: out_unit=101, bf_unit= 102, sa_unit=103, ct_unit=104
        ...
        write(bf_unit,'(3a16)') 'vy','int',''
        call open_output_file(sa_unit,'.sa')
        write(sa_unit,'(7a16)') 'vy','i_s','','i_t','','diff',''
        call open_output_file(out_unit,'.dat')
        call open_output_file(ct_unit,'.steps')
        write (out_unit,'(12a12)') 'omega','','i_bf','','i_simp','',&
                                'i_trap','','i_bf-i_simp','','i_simp-i_trap',''
        write (ct_unit,'(3a12)') 'nsteps_vy','nsteps_vz','nsteps_in'

        call integrator(ky,kz,seed1,integral1)
        nsteps_vy = 0
        nsteps_vz = 0
        nsteps_in = 0
        call simpson_adaptive(seed1,integral2,tol,integral3)
        write (out_unit, '(12e12.4)') seed1, integral1, integral2, integral3,&
                                        integral1-integral2,integral2-integral3
        write (ct_unit, '(3i12.4)') nsteps_vy,nsteps_vz,nsteps_in
        print*, nsteps_vy, nsteps_vz,nsteps_in
        call close_output_file (out_unit)
        call close_output_file (bf_unit)
        call close_output_file (sa_unit)
        call close_output_file (ct_unit)
        call finish_file_utils

contains

  subroutine simpson_adaptive(omega,integral,tol,integral3)
        ...
        complex (kind=8), intent(out) :: integral,integral3
        ...
        integral3 = 1.0 + theta - theta*int_trap
