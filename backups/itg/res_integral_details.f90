...
!                         print*, ky, kz
                          ...
                          call sa_integral(omega,integral_i,s=5)
                          print*, integral_i
...
subroutine sa_integral(omega,integral,s)
  ...
  subroutine vz_integral(vy,integral,s,omega)
        ...
        if (s==5) integral = integral - res_v1 - res_v2
  ...
  subroutine integrand(vz,vy,omega,s,res1,res2,int_tmp)
        ...
        if (s==5) then
                ...
                res2 =  bj**2*sqrt(1.0/pi/2.0)*vy*exp(-0.5*vy**2-&
                        .5*v2**2)*tmp_res2
                !res1 =  exp(-0.5*v1**2)*tmp_res1
                !res2 =  exp(-0.5*v2**2)*tmp_res2
                ...
                coeff_a = (1.0-omd_kx)*ky/A+omd_kx*kx/A
                coeff_b = kz
                coeff_c = 0.5*((1.0-omd_kx)*ky/A+omd_kx*kx/A)*vy**2-omega
                v0 = -coeff_b/2./coeff_a
                dv = sqrt(coeff_b**2-4.*coeff_a*coeff_c)/coeff_a
                !alpha = atan(aimag(dv)/real(dv))
                !print*, 'alpha'
