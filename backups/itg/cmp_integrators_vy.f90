program cmp_integrators_vy
        ...
        write (out_unit,'(8a12)') 'omega','',&
                        'integral1','','integral2','','diff',''
        call integrator(ky,kz,seed1,integral1)
        call simpson_adaptive(vy_grid(1),vy_grid(nvy),seed1,integral2,tol)
        write (out_unit, '(8e12.4)') seed1, integral1, integral2, &
                                integral1-integral2
        call close_output_file (out_unit)
        call close_output_file (bf_unit)
        call close_output_file (sa_unit)
        write(sa_unit,'(7a12)') 'x','','i_simp','','i_trap','','diff',''
        call finish_file_utils

contains

  subroutine simpson_adaptive(a,b,omega,integral,tol)
        ...
        x_left = a
        x_right = x_left + dx
        f_left = integrand(x_left,omega) 
        integral = 0.0
        int_trap = 0.0

        do while (x_left<b)
                x_center = 0.5*(x_left+x_right)
                f_center = integrand(x_center,omega)
                f_right = integrand(x_right,omega)
                ...
                write(sa_unit,'(7e12.4)') x_right, integral, int_trap,&
                                                integral-int_trap
                x_left = x_right
                dx = 0.5*dx*(abs(i_trapezoid-i_simpson)/tol)**(-1.0/3.0)
                if (dx>0.2) dx=0.2
  ...
  function integrand(vy,omega)
        ...
        do iz = 1, nvz
           omega_star_t = 0.5*(-3.0+vy**2+&
                        (vz_grid(iz)**2-zi*2.0*vz_grid(iz)*vi-vi**2))*tprime*ky        
           omega_star_d = (0.5*vy**2+&
                        vz_grid(iz)**2-zi*2.0*vz_grid(iz)*vi-vi**2)*omd*ky/A
           omega_parallel = kz*(vz_grid(iz)-zi*vi)
  ...
  subroutine integrator ( ky, kz, omega, intgral)
	...
	real (kind=8) :: rho, bj, dj, fj
        ...
        do iy = 1, nvy
                ...
                write(bf_unit,'(3e12.4)') vy_grid(iy), intgral_tmp
        end do
  ...
  subroutine read_input_file
        ...
        namelist / parameters / nvy,nvz,lvy,lvz,A,theta,&
