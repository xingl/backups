program imp_tor_itg
        ...
        integer (kind=8) :: nkz, nky, nkx, ntheta_fs
        real (kind=8) :: dkz, dky, dkx, dtheta_fs
        integer (kind=8) :: ikz, iky, ikx, itheta_fs
        real (kind=8) :: ky_start, kz_start, kx_start, theta_fs_start
        ...
        real (kind=8) :: vi, tol, fr, na_e, na_z, na_i
        real (kind=8) :: theta, Zeff, Z, mu_e, mu_z, theta_fs
        ...
        integer :: nlp
        complex (kind=8) :: root, root_kz, root_ky_kz, root_theta_fs
        real(kind=8) :: kx_ref
        ...
        write (gamma_unit,'(9a12)') "theta_fs","tprime","fprime","omd","kx",&
                                "ky","kz","omega","gamma"
        call open_output_file(Dmixing_unit,'.datDmixing')
        write (Dmixing_unit,'(10a12)') "theta_fs","tprime","fprime","omd","kx",&
                                "ky","kz","omega","gamma","Dmixing"
        call open_output_file(out_unit,'.dat')
        write (out_unit,'(9a12)') "theta_fs","tprime","fprime","omd","kx",&
                                "ky","kz","omega","gamma"
        ...
        do itheta_fs = 1, ntheta_fs
          theta_fs = theta_fs_grid(itheta_fs)
          print*, "theta_fs"
          print*, theta_fs
          do iomd = 1, nomd
            omd = omd_grid(iomd)
              ...
              do itprime = 1, ntprime
                 ...
                 do iky = 1, nky
                    ...
                    do ikz = 1, nkz
                       ...
                       if (.not.maxval(aimag(omega_lp))==0) write (gamma_unit,&
                           '(9e12.4)') theta_fs,tprime,fprime,omd, &
                           kx_lp(ikx_lp_gamma(1)),ky,kz,omega_lp(ikx_lp_gamma(1))
                       if (.not.maxval(Dmixing_lp)==0) write (Dmixing_unit, &
                           '(10e12.4)') theta_fs,tprime,fprime,omd, &
                           kx_lp_D(ikx_lp_Dmixing(1)),ky,kz,omega_lp(ikx_lp_Dmixing(1)),&
                           Dmixing_lp(ikx_lp_Dmixing(1))

                       if (.not.maxval(aimag(omega_lp))==0) then  
                           kx_ref = kx_lp(ikx_lp_gamma(1))
                           root_ky_kz = omega_lp(ikx_lp_gamma(1))
                           if (ikz==1) then
                              root_kz = omega_lp(ikx_lp_gamma(1))
                              if (iky==1)  root_theta_fs = omega_lp(ikx_lp_gamma(1))
                           end if
                       end if
 ...
 subroutine kx_scan (iloop,kx_maxgam,omega,kx_maxDmix,Dmixing)
       ...
       do ikx = 1, nkx
          ...
          if (ifprime ==1 .and. itprime==1 .and. iomd==1 &
              .and. iky==1 .and. ikz==1 .and. ikx==1 &
              .and. itheta_fs ==1) then
            seed1 = om1_re + om1_im*zi
            seed2 = om2_re + om2_im*zi
          else if (ifprime ==1 .and. itprime==1 .and. iomd==1 &
              .and. ikz==1 .and. iky==1 .and. ikx==1) then
            seed1 = root_theta_fs
            seed2 = root_theta_fs*0.9
          else if (ifprime ==1 .and. itprime==1 .and. iomd==1 &
          ...
          if (steps==nstep .or. aimag(root)<0) then
...
subroutine sa_integral(omega,integral,s)
  ...
  function integrand(vz,vy,omega,s)
        ...
        if (s==2) then
                ...
                omega_star_d = cos(theta_fs)*(0.5*vy**2+&
                        vz**2-zi*2.0*vz*vi-vi**2)*omd*ky/A +&
                        sin(theta_fs)*(0.5*vy**2+vz**2-zi*2.0*vz*vi-vi**2)*&
                        omd*kx/A
                ...
                omega_star_t = -theta*0.5*(-3.0+vy**2+&
                        (vz**2-zi*2.0*vz*vi-vi**2))*tprime*ky
                omega_star_d = -theta*(cos(theta_fs)*(0.5*vy**2+&
                        vz**2-zi*2.0*vz*vi-vi**2)*omd*ky/A +&
                        sin(theta_fs)*(0.5*vy**2+vz**2-zi*2.0*vz*vi-vi**2)*&
                        omd*kx/A)
                ...
                omega_star_t = 1.0/Z*0.5*(-3.0+vy**2+&
                        (vz**2-zi*2.0*vz*vi-vi**2))*tprime*ky
                omega_star_d = 1.0/Z*(cos(theta_fs)*(0.5*vy**2+&
                        vz**2-zi*2.0*vz*vi-vi**2)*omd*ky/A+&
                        sin(theta_fs)*(0.5*vy**2+vz**2-zi*2.0*vz*vi-vi**2)*&
                        omd*kx/A)
  ...
  subroutine read_input_file
        ...
        namelist / parameters / nstep,nkz,dkz,kz_start, &
                                ...
                                nkx, dkx, kx_start, &
                                ntheta_fs, dtheta_fs, theta_fs_start
...
subroutine init_grids
        ...
        allocate(theta_fs_grid(ntheta_fs))

        do itheta_fs = 1, ntheta_fs
                theta_fs_grid(itheta_fs) = itheta_fs*dtheta_fs*pi + theta_fs_start*pi
        end do
end subroutine
