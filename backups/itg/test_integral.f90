...
subroutine sa_integral(omega,integral,s)
  ...
  function integrand(vz,vy,omega,s)
        ...
        integer :: s, n
        real(kind=8) :: v0,alpha,a,b
        complex(kind=8) :: dv,vzprime,c

        n = 0
        if (s==2) then
                a = (1.0-omd_kx)*ky/A+omd_kx*kx/A
                b = kz
                c = 0.5*((1.0-omd_kx)*ky/A+omd_kx*kx/A)*vy**2-omega
                v0 = -b/2./a
                dv = sqrt(b**2-4.*a*c)/a
                alpha = atan(aimag(dv)/real(dv)) + dalpha
