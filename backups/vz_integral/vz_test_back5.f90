...
contains
  ...
  subroutine vz_integral_residue(vy,omega,s,res_v1,res_v2,shift_vz)
        ...
        if (s==2) then
           ...
           !print*, coeff_a, dv, v0
           ! branch cut in Fortran sqrt
           if (real_discrim <0. .and. coeff_a*aimag(omega)<0.) then
              v1 = v0-dv/2.
              v2 = v0+dv/2.
           else
              v1 = v0+dv/2.
           ...
           !tmp_res1 = zi*exp(-v1**2/2.)/coeff_a*coeff_f
           !tmp_res2 = zi*exp(-v2**2/2.)/coeff_a*coeff_g

           !print*, coeff_f, coeff_g

           tmp_res1 = 2.*pi*zi*exp(-v1**2/2.)/coeff_a*coeff_f
           tmp_res2 = 2.*pi*zi*exp(-v2**2/2.)/coeff_a*coeff_g

           !tmp_res1 = 2.*pi*zi*exp(-v1**2/2.)*(coeff_d*v1**2+coeff_e)/coeff_a/(v1-v2)
           !tmp_res2 = 2.*pi*zi*exp(-v2**2/2.)*(coeff_d*v2**2+coeff_e)/coeff_a/(v2-v1)

           !tmp_res1 = zi*exp(-v1**2/2.)*(coeff_d*v1**2+coeff_e)/coeff_a/(v1-v2)
           !tmp_res2 = zi*exp(-v2**2/2.)*(coeff_d*v2**2+coeff_e)/coeff_a/(v2-v1)

        end if

        if (real_discrim <0. .and. coeff_a<0.) then
        ...
        else if (real_discrim <0. .and. coeff_a>0.) then
           if (aimag(v1) > vz_line) then
              shift_vz = 0.
              res_v1 = 0.
              res_v2 = 0.
