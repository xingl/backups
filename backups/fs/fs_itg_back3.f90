program fs_itg
        ...
        write (gamma_unit,'(9a12)') "theta","tprime","fprime","kv","kx",&
                                "ky","kz","omega","gamma"
        call open_output_file(Dmixing_unit,'.datDmixing')
        write (Dmixing_unit,'(8a12)') "theta","tprime","fprime","kv","kx",&
                                "ky","kz","Dmixing"
        call open_output_file(out_unit,'.dat')
        write (out_unit,'(9a12)') "theta","tprime","fprime","kv","kx",&
                                "ky","kz","omega","gamma"

        root = om1_re + om1_im*zi
        do ifs = 1, n_fs, n_skip
           ...
           do iky = 1, nky
              ...
              do ikz = 1, nkz
                  ...
                  if (.not.aimag(omega_kx_max)==-9999.0) &
                      write (gamma_unit,&
                      '(9e12.4)') theta_fs,tprime,fprime,kv, &
                      kx_gam,ky,kz,omega_kx_max
                  if (.not.Dmixing_kx_max==-9999.0) &
                      write (Dmixing_unit, &
                      '(8e12.4)') theta_fs,tprime,fprime,kv, &
...
!              exit
          ...
          !     '(9e12.4)')theta_fstprime,fprime,kv,kx_base(ikx),&
          !      ky,kz,root_kx_scan(1,ikx)
          !end do
          !write(out_unit,*)
