program theta_avg
        ...
        do i = 1, n_length-1
           theta_grid_ext(i) = -theta_grid(n_length-i)
        end do
        do i = 2*n_length, 3*n_length-2
           theta_grid_ext(i) = pi+theta_grid(3*n_length-i-1)
        end do
           ...
           do idelkz = 1, ndelkz
              ...
              do itheta = itheta_left(1), itheta_right(1)
                 if (itheta>(n_length-1).and.itheta<2*n_length) then
                    ...
                    theta = theta_grid(n_length-itheta)
                    gamE = gamE_grid(n_length-itheta)
                 else
                    theta = theta_grid(3*n_length-itheta-1)
                    gamE = gamE_grid(3*n_length-itheta-1)
                 end if
